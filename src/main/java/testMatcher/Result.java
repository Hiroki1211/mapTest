...
public class Result {
	...
	private ExtractClass extractClass;
...
// private ArrayList<PartiallyMatchingResult> partiallyMatchingResultLists = new ArrayList<PartiallyMatchingResult>();
	...
	public void display() {
		...
		for(int matchingNum = 0; matchingNum < matchingResultLists.size(); matchingNum++) {
			...
			ArrayList<MatchingInstance> matchInstanceLists = matchingResult.getMatchingInstanceLists();
			for(int instanceNum = 0; instanceNum < matchInstanceLists.size(); instanceNum++) {
				MatchingInstance matchingInstance = matchInstanceLists.get(instanceNum);
				System.out.print(matchingInstance.getClassName() + ":");
				
				ArrayList<MatchingPath> matchingPathLists = matchingInstance.getMatchingPathLists();
				for(int pathNum = 0; pathNum < matchingPathLists.size(); pathNum++) {
					MatchingPath matchingPath = matchingPathLists.get(pathNum);
					...
					for(int instNum = 0; instNum < instanceLists.size(); instNum++) {
						Instance instance = instanceLists.get(instNum);
		...
		System.out.println("------NOT MATCH Extract------");
		ArrayList<SameExecuteExtractPath> notMatchingExtractLists = extractClass.getSameExcuteExtractPathLists();
		for(int notExtractNum = 0; notExtractNum < notMatchingExtractLists.size(); notExtractNum++) {
			...
			if(!sameExecuteExtractPath.getIsMatch()) {
				ArrayList<Instance> instanceLists = sameExecuteExtractPath.getInstanecLists();
...
//	}
	...
	public void setExtractClass(ExtractClass input) {
		extractClass = input;
	}
